...
test\_network\_function\_version        | v4.1.0                                               | CNF Cert Suite version downloaded. DCI App Agent supports the latest stable version, which is v4.1.0, and also it has backwards compatibility with v3.x.x, however it is not ensured that it is completely compatible with older versions. HEAD version (in the main branch) can be also used, but it is not guaranteed a complete compatibility with the latest unstable changes.
