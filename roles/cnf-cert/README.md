...
test\_network\_function\_version    | v3.1.0              | CNF Cert Suite version downloaded. The DCI App Agent only suppports the latest stable version, which is v3.1.0. HEAD version (in the main branch) can be also used, but it is not guaranteed a complete compatibility with the latest unstable changes.
