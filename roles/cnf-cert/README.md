...
In the job, starting from tnf v4.3.0 in advance, you will see that a new file is submitted to the Files section in the DCI jobs, called `tnf-results-web-page.tar.gz`. If you extract the compressed files in one single place and open `results.html`, you will be able to upload there the `claim.json` file obtained from the execution and then the web application will evaluate the results and present a report of the tests that have passed, been skipped and failed. This is a tool provided by CNF Cert Suite team.
