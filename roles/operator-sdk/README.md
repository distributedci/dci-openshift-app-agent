...
| ---------------------------------- | --------- | ------------ |
| scorecard_operators_to_certify     | undefined | Mandatory, list of operators to be checked with Operator-SDK Suite. You could find an example [here](#operator-end-to-end-certification). |
...
## Variables to define for each operator in scorecard_operators_to_certify
...
Please note that `oc_tool_path` and `opm_tool_path` variables are required to be set.
...
```
scorecard_operators_to_certify:
