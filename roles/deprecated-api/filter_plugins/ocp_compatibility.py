    ...
    def ocp_compatibility(self, after_api, curr_version, junit_ocp_file):
        ...
        for version, status in ocp_versions.items():
            ...
            if status != 'compatible':
                test_case.add_failure_info(f'Workload is not compatible with OCP-{version}')
                test_case.add_error_info(f'Workload utilizes APIs deprecated for OCP-{version}: {ocp_versions[version]}')
