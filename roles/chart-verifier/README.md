...
| submit_results    | boolean        | true              | Creates a pull request on the defined repository. Defaults to `true`             |
| values_file       | string         | undefined         | File with the values to be overriden in the chart. This field is optional       |
| deploy_chart      | boolean        | true              | Deploys the chart to the cluster. Defaults to `true`. This step is mandatory for the certification |
...
The `submit_results` variable is set to true by default for each chart, this will submit the chart to the [openshift-charts/charts](https://github.com/openshift-helm-charts/charts/) repository by creating a pull request if all the tests passed.
...
1. The pull requests that fail the tests need to manually deleted by the partner.
