...
- [Launching the agent](#launching-the-agent)
  ...
  - [Testing a change at the CNF level on an already installed cluster](#testing-a-change-at-the-cnf-level-on-an-already-installed-cluster)
...
The `dci-openshift-app-agent` is packaged and available as a RPM file located in this repository:
...
Other variables that can be used and that are present in the pipelines are:

- `dci_config_dir`, it targets the directory in which the hooks to be executed by the `dci-openshift-app-agent` are present.
- `dci_openshift_app_image`, related to the image to be used to deploy the container in the OCP cluster by the `dci-openshift-app-agent`. Remember to target the local registry in disconnected environments, instead of using public repositories.
...
### Testing a change at the CNF level on an already installed cluster

When you want to test a specific pipeline related to the `dci-openshift-app-agent`, you can do it by doing the following:

1. Remove the cluster in which OCP is currently installed from available resources (e.g. cluster6).

```bash
dci-queue remove-resource 8nodes cluster6
```

2. Execute the check command, putting the reference to the Gerrit Code Review number or the Github Pull Request URL, together with the kubeconfig file of the selected cluster and the pipeline to be tested. Remember the terminal is blocked after doing this, so it is recommended to open a `tmux` session to run it.

```bash
check 20121 $HOME/clusterconfigs-cluster6/kubeconfig fake-cnf 2>&1 | tee /tmp/log.20121
# after pipeline name, you can modify ansible_extravars with the following: <pipeline_name>:ansible_extravars=<variable>:<value>
```

3. When finishing, do not forget to add the cluster to the available resources:

```bash
dci-queue add-resource 8nodes cluster6 (when you finished with the cluster)
```
