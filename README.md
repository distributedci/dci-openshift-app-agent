...
dci-auth is using HMAC auth v4. Signature are computed on the client side using the UTC date of the server. The signature is computed again on DCI server. To avoid replay attack, the signature is only valid for a few minutes if you are running a job and see MAC errors, for example: `Hmac2Mechanism failed: signature is expired`, the server where the DCI agent is executed may be desynchronised.

A method to confirm, this is the issue, is to execute the command `dcictl topic-list` after sourcing the remote-ci credentials to list the topics, you mignt see `None` as result:
