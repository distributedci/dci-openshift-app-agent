...
dci\_must\_gather\_images          | []                                                   | Extra must-gather plugin images for log collection. Hints (e.g. "ptp", "acm") are resolved from installed CSVs. Full image references (containing "/") are passed through. Unresolved hints are omitted. Every collection also uses the cluster ImageStream `openshift/must-gather:latest`.
...
eco\_test\_suites                  | false                                                | List of eco-gotests test suites to perform: ['ptp','sriov']
