...
dci_comment:
```
2. The DCI OpenShift App Agent by default runs a series of Ansible playbooks called hooks in phases (see section [Hooks](#hooks)). The default files only contain the string `---` and no actions are performed. The install.yml is missing on purpose, and it you run the agent at his point, you will receive an error, you can choose between one of the following option to proceed:
...
- Include dci_config_dir variable in `settings.yml` with the path where are located the hooks you want to execute.
