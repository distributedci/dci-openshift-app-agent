...
all\_registries\_creds             | ""                                                   | Path to the pull-secret.txt file with authentication for all the registries that require an authorized access. Must be set both in connected and disconnected environments if there are private registries to use. Note that this variable is not defined in this agent, but all the checks regarding the variable uses "" as default value.
partner\_creds                     | ""                                                   | Authfile for partner's private registries. Note that this variable is not defined in this agent, but all the checks regarding the variable uses "" as default value.
