...
Some test suites may require a local registry to load/pull container images. The Openshift App agent supports enabling the OCP integrated registry on the current cluster, and configure it with ephemeral storage, so all images are lost if you restart the registry. This configuration is not recommended for production environments.
