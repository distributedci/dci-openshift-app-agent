...
In order to execute the this playbook the following requirement should be available:
...
A directory with registry access credentials will be created in the current play directory.
...
$ podman push default-route-openshift-image-registry.apps.<cluster_name>.<domain>/<project>/image:latest
```
