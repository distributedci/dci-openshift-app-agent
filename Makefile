...
install:
	...
	for hook in hooks/*.yml; do install -p -D -m 644 $$hook  $(BUILDROOT)$(SYSCONFDIR)/dci-openshift-app-agent/$$hook; done
	...
	for play in plays/*.yml; do install -p -D -m 644 $$play $(BUILDROOT)$(DATADIR)/dci-openshift-app-agent/$$play; done

	for script in plays/scripts/*; do install -p -D -m 755 $$script $(BUILDROOT)$(DATADIR)/dci-openshift-app-agent/$$script; done
